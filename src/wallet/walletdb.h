...
public:
    ...
    bool WriteZVYGSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZVYGSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZVYGSeed_deprecated(uint256& seed);
    bool EraseZVYGSeed();
    bool EraseZVYGSeed_deprecated();

    bool WriteZVYGCount(const uint32_t& nCount);
    bool ReadZVYGCount(uint32_t& nCount);
