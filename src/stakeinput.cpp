...
// The zVYG block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zVYG is a hash of the serial
...
{
    //Create an output returning the zVYG that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZVYGOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zVYG output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zVYG", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZVYGOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zVYG output", __func__);
...
{
    CzVYGTracker* zvygTracker = pwallet->zvygTracker.get();
    CMintMeta meta;
    if (!zvygTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zvygTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a VYG stake is the outpoint
