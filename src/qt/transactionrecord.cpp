...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZVYGModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zvygTracker->HasMintTx(hash))) {
            //zVYG stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZVYG;
...
bool IsZVYGType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZVYG:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZVYG || type == TransactionRecord::MNReward) {
