...
#include "ui_zvygcontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zvygTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zvygTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zVYG.";
            else if (!mint.isSeedCorrect)
                strReason = "The zVYG seed used to mint this zVYG is not the same as currently hold in the wallet";
            else
