...
CzVYGTracker::CzVYGTracker(std::string strWalletFile)
{
...
CzVYGTracker::~CzVYGTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzVYGTracker::Init()
{
...
bool CzVYGTracker::Archive(CMintMeta& meta)
{
...
bool CzVYGTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzVYGTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzVYGTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzVYGTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzVYGTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzVYGTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzVYGTracker::GetSerialHashes()
{
...
CAmount CzVYGTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzVYGTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzVYGTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzVYGTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzVYGTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzVYGTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzVYGTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzVYGTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzVYGTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzVYGTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzVYGTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzVYGWallet* zVYGWallet)
{
    bool iszVYGWalletInitialized = (NULL != zVYGWallet);
    CMintMeta meta;
    ...
    if (! iszVYGWalletInitialized)
        zVYGWallet = new CzVYGWallet(strWalletFile);
    meta.isSeedCorrect = zVYGWallet->CheckSeed(dMint);
    if (! iszVYGWalletInitialized)
        delete zVYGWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzVYGTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzVYGTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzVYGTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzVYGTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzVYGTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzVYGTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzVYGWallet* zVYGWallet = new CzVYGWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zVYGWallet);
        }
        delete zVYGWallet;
        LogPrint("zero", "%s: added %d dzvyg from DB\n", __func__, listDeterministicDB.size());
    }
