...
CzVYGWallet::CzVYGWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zvyg seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZVYGSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZVYGSeed_deprecated()) {
                    LogPrintf("%s: Updated zVYG seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zvyg seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zvyg wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzVYGWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZVYGCount(nCountLastUsed);
    else if (!walletdb.ReadZVYGCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzVYGWallet::Lock()
{
    seedMaster = 0;
}

void CzVYGWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzVYGWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZVYG(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzVYGWallet::LoadMintPoolFromDB()
{
...
void CzVYGWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzVYGWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzVYGWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zvygTracker->HasPubcoinHash(pMint.first)) {
...
bool CzVYGWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZVYG(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zvygTracker which also adds to database
    pwalletMain->zvygTracker->Add(dMint, true);
...
void CzVYGWallet::SeedToZVYG(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzVYGWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzVYGWallet::GenerateDeterministicZVYG(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzVYGWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzVYGWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzVYGWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
